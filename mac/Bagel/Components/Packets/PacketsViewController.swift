...
class PacketsViewController: BaseViewController {
  ...
  static let durationColumnWidth = CGFloat(80.0)
  ...
  func setupTableViewHeaders() {
    ...
    for tableColumn in self.tableView.tableColumns {
      ...
      case TableIdentifiers.url:
        ...
        tableColumn.width = self.view.frame.size.width - PacketsViewController.statusColumnWidth - PacketsViewController.dateColumnWidth - PacketsViewController.methodColumnWidth - PacketsViewController.durationColumnWidth
...
extension PacketsViewController: NSTableViewDelegate, NSTableViewDataSource {
  ...
  func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
    ...
    guard let item = self.viewModel?.item(at: row) else { return nil }
    ...
    case TableIdentifiers.date:
      ...
      cell.model = .init(packet: item, mode: .date)
      cell.backgroundStyle = .normal
      return cell
    case TableIdentifiers.duration:
